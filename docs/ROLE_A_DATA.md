...
**축은 원본 데이터를 그대로 따른다 — 변환하지 않는다** (2026-08-28 정정. 이전 안의
"5세 단위 / 4시간 균등"은 원본을 열어 보기 전의 가정이었다).

| 축 | 값 |
|---|---|
| `age_band` | **10년 단위** — 10 · 20 · 30 · 40 · 50 · 60. 60대 이상은 `60`으로 접는다 |
| `hour_band` | **불균등 6구간** — 0=00\~06 · 1=06\~11 · 2=11\~14 · 3=14\~17 · 4=17\~21 · 5=21\~24 |
| `dow_type` | 평일 `0` / 주말 `1` |

`age_band`에 `CHECK` 제약이 걸려 있다(`db/migrations/003_segment_axis.sql`).
5세 단위로 넣으면 INSERT가 실패한다. 축이 어긋난 채 적재되면 B의 조회가 0행이
되고 **에러 없이 개인화 항(0.22)만 사라지기** 때문에, 일부러 시끄럽게 막아 뒀다.
