...
# 용산 지점들의 평균 연령 비율. live_segment_match의 분모다.
#
# 실측치다. A가 `hotspot_snapshot.age_rates` **2,122 스냅샷**을 인구
# 가중평균해 계산했다 (roleA/reports/coverage.md §9). 이전 값은 설계 시점의
# 잠정치였고, 10대에서 2.1배(0.10 vs 0.0477) 어긋나 있었다 — 분모가 크면
# live_segment_match가 그만큼 낮게 나오므로, 10대 사용자는 또래가 실제로
# 많은 지점에서도 이 항이 오르지 않았다.
#
# ⚠️ 합이 1이 아니다(0.9326). citydata는 0~9세와 70대 이상도 주는데 엔진의
#    age_band 계약이 10~60이라 그 두 밴드가 빠진 값이다. **재정규화하지 않는다** —
#    분자(`age_rates[age_band]`)도 같은 전체 인구 대비 비율이므로, 분모만
#    10~60 합으로 다시 맞추면 두 값의 기준이 어긋나 비율이 부풀려진다.
BASELINE_AGE_RATE: dict[int, float] = {
    10: 0.047717, 20: 0.272665, 30: 0.226391,
    40: 0.167909, 50: 0.133623, 60: 0.084339,
}
